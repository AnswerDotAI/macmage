...
The Imp boundary follows from Imp's purpose (a shared TCC anchor everyone auths once): Imp hosts only what its bundle uniquely enables - the permission machinery, being the responsible parent that spawns things, and the UI macOS refuses to unbundled processes (wisps, notifications). Everything else, including loopy/eventy capabilities that need their own main loop, belongs in macmage as a fresh *Python* child process: any descendant of the agent inherits Imp's grants, and `multiprocessing`'s spawn context gives real functions in fresh interpreters with pipes, queues, and exception propagation, no command lines involved (never `python -c`). `apart(fn, *args)` in util.py wraps that once, and `snap` runs on it; `Imp --snap` stays for now as the sample bytes-over-stdout Swift verb, until another replaces it. When Apple ships a needed Swift-only API, the shape is a helper spawned under Imp, not more surface on Imp.
